...
- CSVデータは公開ディレクトリに配置
- 機密情報（個人情報、口座番号等）を含まない
- 必要に応じてBasic認証等のアクセス制限を検討

### 9.2 入力検証

- 年度パラメータの検証（数値型、範囲チェック）
- CSVデータの形式検証
- XSS対策（React/Next.jsの標準機能に依存）
...
A: はい、`topN`パラメータで変更可能です。デフォルトは10件ですが、設定で変更できるように実装します。

### Q2: 複数年のポートフォリオ比較はできるか？

A: 本仕様では単年表示のみですが、将来的な拡張として検討しています。Phase 3以降で実装する可能性があります。

### Q3: 銘柄の色は固定されるか？

A: 基本的にRechartsの自動割り当てですが、特定の銘柄に固有色を設定する機能も検討しています。

### Q4: 円グラフ以外の表示形式は？

A: 本仕様では円グラフのみですが、将来的にツリーマップやバーチャート等の表示形式を追加する可能性があります。

### Q5: 配当データのエクスポートは可能か？

A: Phase 3でCSV/PNG形式のエクスポート機能を検討しています。
