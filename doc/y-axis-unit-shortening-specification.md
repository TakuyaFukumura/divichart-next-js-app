...
- **要件**: y軸の数値を日本語の単位（万、千）を使って短縮表示する
- **優先度**: 必須
- **詳細**:
  - 10,000円以上の金額は「万円」単位で表示
  - 1,000円以上10,000円未満の金額は「千円」単位で表示（オプション）
...
- **優先度**: 必須
- **詳細**:
  - y軸は短縮表示、ツールチップは完全な金額（例: ¥320,000）
...
| 0円 | 0 | 0 | 0 |
| 1円〜999円 | N円 | 500 | 500円 |
| 1,000円〜9,999円 | N.N千円 または N千円 | 5,000 | 5千円 |
| 10,000円〜999,999円 | N万円 または N.N万円 | 50,000 | 5万円 |
...
- 1,000円以上10,000円未満: 千円単位（オプション、小数点なし）
- 1,000円未満: 円単位
...
- 10,000円以上: 万円単位（必要に応じて小数点1桁）
- 1,000円以上10,000円未満: 千円単位（小数点1桁）
- 1,000円未満: 円単位
...
export function formatYAxisValue(value: number): string {
    ...
    if (absValue >= 10000) {
        ...
        if (manValue % 1 === 0) {
            ...
            return `${sign}${manValue.toFixed(1)}万円`;
        }
    }

    // 1千円以上1万円未満の場合は千円単位で表示（オプション）
    if (absValue >= 1000) {
        const senValue = absValue / 1000;
        if (senValue % 1 === 0) {
            return `${sign}${senValue}千円`;
        } else {
            return `${sign}${senValue.toFixed(1)}千円`;
        }
    }
...
#### 代替案1: 「K」「M」表記の継続（英語圏表記）

- **内容**: 現在の累計配当グラフで使用されている「K」（thousand）、「M」（million）表記を継続
- **メリット**: 国際的に認知された表記
- **デメリット**: 日本のユーザーには馴染みが薄い、直感的でない
- **採用判断**: 不採用（日本語表記の方が優れている）

#### 代替案2: toLocaleString()のみ使用（カンマ区切り）

- **内容**: 「320,000」のようにカンマ区切りのみで表示
- **メリット**: 正確な金額表示
- **デメリット**: 桁数が多く、y軸の幅を取る
- **採用判断**: 不採用（短縮化の目的を達成できない）

#### 代替案3: 動的な単位選択（グラフの範囲に応じて自動判定）

- **内容**: グラフの最大値に応じて「千円」「万円」「億円」を自動選択
- **メリット**: 常に最適な単位で表示
- **デメリット**: 実装が複雑、異なるグラフ間で表記が統一されない可能性
- **採用判断**: 将来の拡張として検討（現在は固定ルールで実装）
